...
(function(){
 ...
 var JI=["대치","강남1","강남2","송파","강동","하남","신촌","분당","김포","동탄","센텀","수성","본사"];
 ...
 function cardRows(){return CARDS.map(function(n){return '<div style="display:flex; align-items:center; gap:6px; min-width:0"><span style="flex:1; min-width:0; font-size:13px; color:var(--text-secondary); white-space:nowrap">'+n+'</span><input type="number" min="0" step="120" class="po-cqty po-num" data-type="'+n+'" placeholder="120단위" style="width:56px"><span class="po-unit">장</span></div>'}).join('')}
 ...
 function nearest(v,a){var b=a[0],bd=Infinity;a.forEach(function(x){var d=Math.abs(x-v);if(d<bd){bd=d;b=x;}});return b;}
 function snapBlock(block){
  var item=block.querySelector(".po-item").value;
  if(item==="사감 메세지카드"){block.querySelectorAll(".po-cqty").forEach(function(i){var v=Number(i.value);if(v>0)i.value=String(Math.round(v/120)*120);});}
  else if(item==="냉장고 스티커"||item==="경고 스티커"){block.querySelectorAll(".po-qty,.po-bulk").forEach(function(i){var v=Number(i.value);if(v>0)i.value=String(nearest(v,[500,1000,2000]));});}
 }
 function recount(){
  var n=0,tot=0;
 ...
 host.addEventListener("change",function(e){var b=(e.target&&e.target.closest)?e.target.closest(".po-block"):null;if(b){snapBlock(b);recount();}});
 submitBtn.addEventListener("click",function(){
  ...
  host.querySelectorAll(".po-block").forEach(function(b){
   ...
   head+=(pay==="split")?" · 지점별 따로 결제":" · 묶어서 한 번에 결제";
